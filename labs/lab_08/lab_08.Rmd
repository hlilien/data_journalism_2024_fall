...
md_ballots_date <- read_csv("data/md_absentee_ballots_date.csv")
```
...
```{r}
md_ballots |>
  mutate(dem_percent_return = `DEM RECEIVED` / DEM_SENT * 100) |>
  arrange(dem_percent_return) |>
  slice(1:10) |>
  ggplot(aes(x = reorder(`COUNTY NAME`, dem_percent_return), y = dem_percent_return)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(
    title = "Top 10 Maryland Counties by Percent of Absentee Ballots Returned by Democrats",
    y = "Percent of Absentee Ballots Returned by Democrats",
    x = "County",
    caption = "Source: Maryland State Board of Elections"
  )
...
md_ballots_date |>
  ...
  labs(
    title = "Absentee Ballot Returns Over Time in Frederick County",
    x = "Date",
...
md_ballots_date |>
  ...
  geom_text_repel(aes(x = `TOTAL SENT`, y = `TOTAL RECEIVED`, label = `COUNTY NAME`),
                  max.overlaps = Inf,
                  size = 3,
                  box.padding = 0.5) +
