...
```{r}
# Party registrations
 party_registrations <- read.csv("data/new_registrations_by_party.csv")
...
```{r}
# Removals
removals <- read.csv("data/removals_by_party.csv")
...
```{r}
# Changes
changes <- read.csv("data/changes_in_registration.csv")
...
current_registrations <- read.csv("data/current_registrations.csv")
...
```{r}
# Party registrations
party_registrations_September <- read.csv("data/new_registrations_by_party_Sep.csv")

party_registrations_September
```

```{r}
# Removals
removals_September <- read.csv("data/removals_by_party_Sep.csv")
...
```{r}
# Changes
changes_September <- read.csv("data/changes_in_registration_Sep.csv")
...
```{r}
# Current registrations
current_registrations_September <- read.csv("data/current_registrations_Sep.csv")

current_registrations_September
```
...
A1. In September 2024, there was a noticeable increase in unaffiliated voters compared to 2022, with Democrats and Republicans seeing smaller changes. The rise in unaffiliated voters is the most interesting trend.

```{r}
percent_2024 <- current_registrations_September |>
  mutate(perc_dem = dem/total*100,
         perc_rep = rep/total*100,
         perc_unaf = unaf/total*100
         ) |>
  select(county, perc_dem, perc_rep, perc_unaf)
```
...
A2. Montgomery County had the biggest increase in unaffiliated voters between 2022 and 2024. It leaned Democrat in the 2022 governor and 2020 presidential elections. Compared to Frederick County, which had a smaller increase, Montgomery is more Democratic, while Frederick is more balanced.
...
A3.New voter registrations didn’t match the overall makeup in 2022 and 2024. More new unaffiliated voters were registered than expected, especially through online methods.

```{r}
percent_2022 <- current_registrations_September |>
...
A4.Comparing 2022 to 2024, Montgomery County saw the biggest rise in unaffiliated voters, while Frederick County's increase was smaller. This suggests a growing trend toward voter independence in more urban areas.

```{r}
percent_2022 |>
  inner_join(percent_2024) |>
  muatate(perc_change_dem = (perc_dem_2024-perc_dem_2022)/perc_dem_2022*100,
...
A5. The most interesting story idea is the rise in unaffiliated voters.
