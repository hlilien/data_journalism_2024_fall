...
**The first line of the red warning message states the following: "One or more parsing issues, call `problems()` on your data frame for details"**
...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **According to the data, there were 1,640 rows, meaning 1,640 problems. I think the problem has something to do with true/false data and determining which is which.**
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **There is a 'chr' delimiter shown in red, which could be considered an error.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **To me, the district field is unrecognized data because the only thing it says is 'N/A'. I think the same for the mail column.. just unrecognized, undelivered data. With the precinct field, I think this is data depending on character types.**
...
**There are 402345 where mail is N/A. I am not sure how to count the number of precinct results**
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **The 'Legacy Data' reason interests me because I really don't know what that is so I want to learn more about it.**
...
**There are 21 rows in this demonstration.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **This type of data is a character string.**
...
**Common issues with data sets that could prevent accurate analysis include missing data, inconsistent formatting, and outliers or incorrect data, all of which can lead to incomplete or skewed results.**
...
**The clean_names function from the janitor package simplifies column names by making them lowercase, replacing special characters with underscores, and fixing names that start with numbers.This makes the data easier to work with.**
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **The error below says the following: invalid 'type' (character) of argument**
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **<dbl>**
...
**Task** Run the following codeblock to add up the amount of all contributions in this data. What is the total? Answer below. **The total is 226**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **There are 2 duplicate rows. The donor name is Kathy Barnette**
...
**This code cleans the dataset by standardizing column names, renaming x1_linenumber to linenumber, and removing duplicate rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **This make all of the data much more consistent.**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **None to my knowledge.**
