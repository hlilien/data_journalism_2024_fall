...
**Task** Load the five CSV files. Do they have the same column structure? **They have the same column structure.**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **24 rows. There should be 24 rows.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **The message says that the join was done on the "county" column.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Somerset County.**
...
**Montgomery County has the highest 18+ Asian population percentage. Montgomery has a higher percentage of Asian residents compared to the previous task, where Somerset had the highest Black population percentage.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Im not sure**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **I do know know.**
