...
**Task** Run the following code to load some campaign expenditure data focused on food-related expenses in Washington, D.C. **The data was successfully loaded, and shows food-related campaign expenses in D.C.**
...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **There's inconsistent capitalization, preventing proper grouping.**
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Standardized capitalization, merged similar names, and removed punctuation for more consistency.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **The method focused more on fixing spelling similarities, and while it was a bit more thorough, the results were pretty similar to the first method used.**
